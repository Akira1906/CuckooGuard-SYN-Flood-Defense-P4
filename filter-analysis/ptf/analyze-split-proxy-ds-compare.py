...
class UnitTest(DemoTumTest):
    ...
    def test_simple_deletion_cuckoo(self):
        
        client_port = self.client_port -1
        
        # Step 1: trigger deletion from Cuckoo Filter

        print("\n[INFO] Testing Cuckoo Filter Deletion...")

        # packet from ebpf to P4 signaling to delete the connection to the Cuckoo Filter
        ...
        # Step 4: try to send legitimate TCP packet through P4 check if it gets through, if not Cuckoo Filter operation was successfull
    ...
    def mass_test_cuckoo(self):
        print("\n[INFO] This test is not deterministic and therefore expected to fail sometimes...")
        print("\n[INFO] Mass Testing Cuckoo Filter Insertion...")

        for i in range(100):
            client_port = self.client_port + i + 1

            # Step 1: trigger insertion into Cuckoo Filter
            ack_pkt = (
            Ether(dst=self.server_mac, src=self.switch_server_mac, type=0x0800) /
            IP(src=self.client_ip, dst=self.server_ip, ttl=64, proto=6, id=1, flags=0) /
            TCP(sport=client_port, dport=self.server_port, flags="E", seq=1, ack=38, window=502)
            ...
            tu.send_packet(self, self.ebpf_iface, ack_pkt)
            sleep(0.01)
            # Step 2: try to send legitimate TCP packet through P4 check if it gets through, if yes Cuckoo Filter operation was successful
            tcp_load = b"GET /index.html HTTP/1.1\r\nHost: 10.0.1.3\r\n\r\n"
            ack_pkt = (
            Ether(dst=self.switch_client_mac, src=self.client_mac, type=0x0800) /
            IP(src=self.client_ip, dst=self.server_ip, ttl=64, proto=6, id=1, flags=0) /
        ...
        print("\n[INFO] Mass Testing Cuckoo Filter Deletion...")
        ...
        for i in range(50, 100):
            ...
            Ether(dst=self.switch_server_mac, src=self.server_mac, type=0x0800) /
            IP(src=self.server_ip, dst=self.client_ip, ttl=64, proto=6, id=1, flags=0) /
            TCP(sport=self.server_port, dport=client_port, flags="UEA", seq=1, ack=38, window=502)
            )
            
            tu.send_packet(self, self.ebpf_iface, ack_pkt)
            sleep(0.01)
            # Step 2: try to send legitimate TCP packet through P4 check if it gets through, if not Cuckoo Filter operation was successful
            tcp_load = b"GET /index.html HTTP/1.1\r\nHost: 10.0.1.3\r\n\r\n"
            ack_pkt = (
            Ether(dst=self.switch_client_mac, src=self.client_mac, type=0x0800) /
            IP(src=self.client_ip, dst=self.server_ip, ttl=64, proto=6, id=1, flags=0) /
        ...
        print("\n[INFO] Testing Cuckoo Filter Capacity...")
        
        successfull_capacity = 0
        ...
        for i in range(100, 400):
            ...
            TCP(sport=client_port, dport=self.server_port, flags="PA", seq=1, ack=32454) /
            Raw(load=tcp_load)
            )# fingerprint value: index 249 with value 426
            # index 159: 426
            # index 
            ...
            try:
                ...
                print(f"[ERROR] Cuckoo Filter insertion failed for port {client_port}. Current items in filter: {i - 50 - 1}")
                # this fails because the elements have been shifted around a lot and there are fingerprint collisions as well I guess
                successfull_capacity = i - 50 - 1
                break
        
        if successfull_capacity > 0.92 * 316:
            print(f"\n[INFO] Cuckoo Filter capacity test passed with {successfull_capacity} items.")
        else:
            raise RuntimeError(f"Cuckoo Filter capacity test failed. Only {successfull_capacity} items inserted successfully.")
