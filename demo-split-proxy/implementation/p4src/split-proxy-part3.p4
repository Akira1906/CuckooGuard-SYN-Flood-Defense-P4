...
control SwitchIngress(
    ...
    register<bit<1>>(BLOOM_STAGE_SIZE) reg_bloom_0_3;

    // Bloom Filter #1
    register<bit<1>>(BLOOM_STAGE_SIZE) reg_bloom_1_1;
    register<bit<1>>(BLOOM_STAGE_SIZE) reg_bloom_1_2;
    register<bit<1>>(BLOOM_STAGE_SIZE) reg_bloom_1_3;
    ...
    action set_bloom_3_a(){
        // hash(meta.bloom_hash_3, HashAlgorithm.crc32, (bit<32>)0, 
        //     {hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.tcp.src_port, hdr.tcp.dst_port}, 
        //     (bit<32>)STAGE_SIZE_MINUS_ONE);
        ...
        reg_bloom_0_3.write(meta.bloom_hash_3, (bit<1>) 1);
        reg_bloom_1_3.write(meta.bloom_hash_3, (bit<1>) 1);
    }
    ...
    action read_sync_bloom_3(){
        reg_bloom_0_3.read(meta.bloom_read_3, meta.bloom_hash_3);
        if (meta.bloom_read_3 == 0) {
            reg_bloom_1_3.read(meta.bloom_read_3, meta.bloom_hash_3);
            // either write 0 in it again or write 1
            reg_bloom_0_3.write(meta.bloom_hash_3, (bit<1>) meta.bloom_read_3);
        } else {
            reg_bloom_1_3.write(meta.bloom_hash_3, (bit<1>) meta.bloom_read_3);
        }
    }
    ...
    apply {
        ...
        if(hdr.tcp.isValid() && standard_metadata.ingress_port == SERVER_PORT && hdr.tcp.flag_ece==1){
            ...
            get_bloom_2_a();
            get_bloom_3_a();
            if(meta.bloom_read_1==1 && meta.bloom_read_2==1 && meta.bloom_read_3==1){
