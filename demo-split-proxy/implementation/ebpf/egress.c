...
int tc_egress(struct __sk_buff *skb)
{
    ...
    if (DEBUG)
        bpf_trace_printk("ENTERING EGRESS!");
    ...
    {
        if (DEBUG)
            bpf_trace_printk("TC_EGRESS: UDP PACKET %x", ip->protocol);
        ...
        if (DEBUG)
            bpf_trace_printk("egress src port  %d", ntohs(udp->source));
        if (DEBUG)
            bpf_trace_printk("egress dst port %d", ntohs(pkt.dst_port));
        ...
        if (ntohs(pkt.dst_port) == TS_UDP_PORT)
        {
            ...
            if (DEBUG)
                bpf_trace_printk("EGRESS 32b SYS TIME 0x%x", ts);
    ...
    {
        if (DEBUG)
            bpf_trace_printk("TC_EGRESS: TCP PACKET %x", ip->protocol);
        ...
        else
        {
            ...
            if (tcp->ack == 0x1 && tcp->syn == 1)
            {
                ...
                if (DEBUG)
                    bpf_trace_printk("ORIG COOKIE + 1 IS 0x%x", old_ack_seq);
                // delta = c - y
                if (DEBUG)
                    bpf_trace_printk("y + 1 is 0x%x", ntohl(tcp->ack_seq));
                ...
                if (DEBUG)
                    bpf_trace_printk("original tcp cksum before update is 0x%x", ntohs(tcp->check));
                tcp->check = eg_compute_full_tcp_checksum(ip, tcp_seg_full, data, data_end);
                if (DEBUG)
                    bpf_trace_printk("TC_EGRESS: updated tcp checksum for packet after processing is 0x%x", ntohs(tcp->check));
                ...
                if (DEBUG)
                    bpf_trace_printk("ip csum offset is 0x%x", IP_CSUM_OFF);
                ret = bpf_l3_csum_replace(skb, sizeof(*eth) + IP_CSUM_OFF, old_ip_len_n, new_ip_len_n, sizeof(new_ip_len_n));
                if (ret < 0)
                {
                    if (DEBUG)
                        bpf_trace_printk("bpf_l3_csum_replace failed");
                    goto DROP;
                }
                // write the new total_len to packet
                if (DEBUG)
                    bpf_trace_printk("ip total len offset is 0x%x", IP_TOTLEN_OFF);
                ...
                if (DEBUG)
                    bpf_trace_printk("orig chksum value is 0x%x", ntohs(tcp->check));
                if (DEBUG)
                    bpf_trace_printk("old flags *value is 0x%x", ntohs(*old_flags_ptr));
                ...
                {
                    if (DEBUG)
                        bpf_trace_printk("bpf_l4_csum_replace failed");
                    goto DROP;
                }
                ...
                {
                    if (DEBUG)
                        bpf_trace_printk("store bytes failed");
                    goto DROP;
                }
                if (DEBUG)
            ...
            else
            {
                ...
                if (DEBUG)
                    bpf_trace_printk("In map, non-SYN-ACK packet");
                if (DEBUG)
                    bpf_trace_printk("Current map state is 0x%x", map_val.map_state);
                uint32_t old_seq_n = tcp->seq;
                if (DEBUG)
                    bpf_trace_printk("orig chksum value is 0x%x", htons(tcp->check));
                if (DEBUG)
                ...
                if (DEBUG)
                    bpf_trace_printk("size of new_seq_n 0x%x", sizeof(new_seq_n));
