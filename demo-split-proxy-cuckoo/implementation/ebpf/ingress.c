	...
	if (ip->protocol == IPPROTO_TCP)
	{
		...
		*/
		...
		if (map_val_ptr == NULL)
		{
			...
			{
				if (DEBUG)
					bpf_trace_printk("XDP_INGRESS: We have ourselves an UNTAGGED packet!");
			...
			if (DEBUG)
				bpf_trace_printk("XDP_INGRESS: We have ourselves a TAGGED packet!");
			// store initial ack value to get delta later
			if (DEBUG)
				bpf_trace_printk("Ingress: orig c + 1 in network order: 0x%x", tcp->ack_seq);
			if (DEBUG)
				bpf_trace_printk("Ingress: orig c + 1 in host order: 0x%x", ntohl(tcp->ack_seq));
			map_val.delta = ntohl(tcp->ack_seq);
			if (DEBUG)
				bpf_trace_printk("current map val, stored in host order? 0x%x", map_val.delta);
			...
			{
				if (DEBUG)
					bpf_trace_printk("XDP_INGRESS: We have ourselves a TAGGED ACK packet!");
				...
				if (DEBUG)
					bpf_trace_printk("XDP_INGRESS: current flags are 0x%x", ntohs(old_flags_n));
				...
				if (DEBUG)
					bpf_trace_printk("XDP_INGRESS: updated flags are 0x%x", ntohs(new_flags_n));
				rc = XDP_PASS;
			...
			{
				if (DEBUG)
					bpf_trace_printk("Ingress: not in map, tagged DATA packet (not tagged ACK)");
				// tcp->ack = 0;
				// tcp->syn = 1;
		...
		{ // connection in map
			...
			if (map_val.map_state != ST_SYN_SENT)
			{
				...
				tcp->check = compute_incr_tcp_checksum(tcp->check, (uint16_t)((old_ack_seq_n >> 16) & 0xFFFF), (uint16_t)((tcp->ack_seq >> 16) & 0xFFFF));
				if (DEBUG)
					bpf_trace_printk("XDP_INGRESS: updated TCP checksum: 0x%x", ntohs(tcp->check));
			} // end if delta update
